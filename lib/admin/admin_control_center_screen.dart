...
List<String> controlCenterSecurityActions(Map<String, dynamic> event) {
  ...
  final bool explicitlyNonActionable =
...
class AdminControlCenterScreen extends StatefulWidget {
  const AdminControlCenterScreen({super.key, this.api, this.initialModuleId});
...
class _AdminControlCenterScreenState extends State<AdminControlCenterScreen> {
  ...
  bool _initialModuleOpened = false;
  ...
  Future<void> _loadCatalog() async {
    ...
    try {
      ...
      final String initialId = widget.initialModuleId?.trim() ?? '';
      if (!_initialModuleOpened && initialId.isNotEmpty) {
        _initialModuleOpened = true;
        _Module? initialModule;
        for (final _Module module in _modules) {
        ...
        if (initialModule == null) {
          throw StateError('Unknown Control Center module: $initialId');
        }
        await _open(initialModule);
        return;
      }
    } catch (e) {
      await _handle(e);
  ...
  String _routeFor(_Module module) => '/control-center/${module.id}';

  void _navigateTo(_Module module) {
    Navigator.of(context).pushNamed(_routeFor(module));
  }

  void _leaveWorkspace() {
    final String? routeName = ModalRoute.of(context)?.settings.name;
    if (routeName != null && routeName.startsWith('/control-center/')) {
  ...
  Widget _home() => ListView(
    ...
    children: <Widget>[
      ...
      LayoutBuilder(
        builder: (_, c) {
          final double width = c.maxWidth > 700
              ? (c.maxWidth - 12) / 2
              : c.maxWidth;
          return Wrap(
            spacing: 12,
                  ...
                  (m) => SizedBox(
                    ...
                    child: Card(
                      child: ListTile(
                        ...
                        onTap: _available(m) ? () => _navigateTo(m) : null,
                      ),
                    ),
                  ),
                )
  ...
  bool _available(_Module module) {
    ...
    final dynamic modules = root is Map
        ? root['modules'] ?? root['capabilities']
        : null;
    if (modules is Map) {
      final dynamic v =
          modules[module.id] ??
                      ...
                      .map(
                        (n) => SizedBox(
                          ...
                          child: TextField(
                            ...
                            decoration: InputDecoration(labelText: _title(n)),
                          ),
                        ),
                      )
                      .toList(),
  ...
  Widget _dates(_Module m) => Wrap(
    ...
    children: <Widget>[
      ...
      ...<String>['Today', '7 Days', '30 Days', 'This Month', 'Custom'].map(
        (p) => ChoiceChip(
          label: Text(p),
          selected: _preset == p,
      ...
      builder: (_) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          ...
          actions: <Widget>[
            ...
            FilledButton(
              onPressed: () {
                if (action != 'REOPEN' && note.text.trim().length < 10) {
                  setDialogState(() {
                    validationError =
  ...
  Widget _executive() {
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        ...((r['recentActivity'] as List? ?? <dynamic>[]).whereType<Map>().map(
          (x) => _logCard(
  ...
  Widget _transaction(List<Map<String, dynamic>> records) {
    ...
    return Column(
      ...
      children: <Widget>[
        const Text(
          'Transaction summary and drill-down',
          style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
        ),
        _kpis(
          r['summary'] is Map
        ...
        if (records.isEmpty)
          ...
          ...records.map(
            (x) => _logCard(
              x,
              _Module('transaction-analytics', '', Icons.insights),
            ),
          ),
      ],
    );
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        if (records.isEmpty)
          ...
          ...records.map(
            (x) => _logCard(x, _Module('customer-analytics', '', Icons.people)),
          ),
      ],
    );
  }
          ...
          return Card(
            child: Padding(
              ...
              child: Column(
                ...
                children: <Widget>[
                  Text(
                    _first(r, const <String>['service', 'source']),
                  ...
                  Text('Operational status: ${r['operationalStatus'] ?? '—'}'),
  ...
  Widget _exportHistory(List<Map<String, dynamic>> rows) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: <Widget>[
      const Text(
        'Export history',
  ...
  Future<void> _export(String dataset, _Module m) async {
    try {
      final ex = await _api.exportDataset(
        dataset,
        from: _range.start,
        to: _range.end,
      ...
      if (!controlCenterDownloadSupported) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('CSV download is supported in a web browser only.'),
            ),
          );
        }
        return;
      ...
      if (ex.csv == null) {
        throw const AdminControlApiException(
          0,
          'The export service did not return CSV data.',
        );
      }
      await downloadControlCenterCsv(
        ex.csv!,
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: <Widget>[
            ...
            TextField(
              controller: subject,
              decoration: const InputDecoration(labelText: 'Subject user ID'),
            ),
            TextField(
