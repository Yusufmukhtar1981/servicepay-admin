...
class LegacyAdminDashboardScreen extends StatefulWidget {
  const LegacyAdminDashboardScreen({
    super.key,
  });

  @override
  State<LegacyAdminDashboardScreen> createState() =>
      _LegacyAdminDashboardScreenState();
}

class _LegacyAdminDashboardScreenState
    extends State<LegacyAdminDashboardScreen> {
  ...
  Future<void> loadDashboard() async {
    ...
    try {
      ...
      final String lowerBody = rawBody.toLowerCase();
      ...
      final Map<String, dynamic> body = toMap(decodedBody);
      ...
      if (body['success'] != true) {
        throw Exception(
          body['message']?.toString() ?? 'Unable to load admin dashboard.',
        );
      }

      final Map<String, dynamic> data = toMap(body['data']);

      final Map<String, dynamic> users = toMap(data['users']);

      final Map<String, dynamic> kyc = toMap(data['kyc']);

      final Map<String, dynamic> wallets = toMap(data['wallets']);

      final Map<String, dynamic> transactions = toMap(data['transactions']);

      final Map<String, dynamic> servicepay = toMap(data['servicepay']);
      ...
      setState(() {
        ...
        totalWalletBalance = toDouble(
          wallets['totalWalletBalance'] ?? wallets['totalBalance'],
        );
        ...
        totalTransactionValue = toDouble(
          transactions['totalVolume'] ?? transactions['totalValue'],
        );

        successfulTransactions = toInt(
          transactions['successful'],
        );

        pendingTransactions = toInt(
          transactions['pending'],
        );

        failedTransactions = toInt(
          transactions['failed'],
        );

        servicepayProfit = toDouble(
          servicepay['totalProfit'] ?? transactions['servicepayProfit'],
        );
        ...
        recentTransactions = data['recentTransactions'] is List
            ? List<dynamic>.from(
  ...
  String formatMoney(
    ...
    final String rounded = amount.toStringAsFixed(2);

    final List<String> parts = rounded.split('.');

    final String formattedWhole = parts[0].replaceAllMapped(
      RegExp(
        r'\B(?=(\d{3})+(?!\d))',
      ),
      (Match match) => ',',
  ...
  Widget buildRecentUsersSection() {
    final List<Widget> userWidgets = recentUsers.map(
      (dynamic rawUser) {
        final Map<String, dynamic> user = toMap(rawUser);

        final String name = user['fullName']?.toString() ?? 'Unknown User';
  ...
  Widget buildRecentTransactionsSection() {
    final List<Widget> transactionWidgets = recentTransactions.map(
      (dynamic rawTransaction) {
        final Map<String, dynamic> transaction = toMap(rawTransaction);

        final Map<String, dynamic> customer = toMap(
          transaction['customerId'],
        );

        final String serviceType =
            transaction['serviceType']?.toString() ?? 'TRANSACTION';
  ...
  Widget buildOverviewGrid(
    ...
    return GridView.count(
      ...
      mainAxisSpacing: 14,
      crossAxisSpacing: 14,
      childAspectRatio: constraints.maxWidth < 600 ? 1.32 : 1.5,
      children: [
        buildStatCard(
          title: 'Total Users',
    ...
    return Column(
      children: [
        ...
        buildAdminTool(
          ...
          subtitle: 'Review and approve customer verification requests.',
          icon: Icons.verified_user_outlined,
          onTap: () {
            openModule(
              title: 'ID Verification',
        ...
        buildAdminTool(
          ...
          subtitle: 'Send direct or broadcast notifications to users.',
                  ...
                  child: LayoutBuilder(
                    builder: (
                      ...
                      final double contentWidth =
                          constraints.maxWidth > 1100 ? 1050 : double.infinity;
