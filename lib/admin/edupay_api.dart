...
class EduPayApi {
  ...
  Future<Map<String, dynamic>> adminPlans({
    String? search,
    String? status,
    String? dateFrom,
    String? dateTo,
  ...
  Future<Map<String, dynamic>> adminTransactions({
    String? search,
    String? status,
    String? dateFrom,
    String? dateTo,
  ...
  Future<http.Response> privateAssetBytes(
    ...
    final response = await client.get(
      Uri.parse(
        '$baseUrl/admin/edupay/schools/$schoolId/private-assets/$fileId',
      ),
      headers: {'Accept': '*/*', 'Authorization': 'Bearer $token'},
    );
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw Exception(
        'Unable to download private asset (${response.statusCode}).',
      );
    }
    return response;
  }
  ...
  Future<Map<String, dynamic>> enableFeature(String feature, String reason) =>
      request(
        'PATCH',
        '/feature-control/admin/$feature',
        body: {'enabled': true, 'reason': reason, 'confirmationText': feature},
      );
}
